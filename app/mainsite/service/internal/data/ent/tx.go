...
func (tx *Tx) init() {
	tx.RdMessage = NewRdMessageClient(tx.config)
	tx.RdMoment = NewRdMomentClient(tx.config)
	tx.RdRMomentsFollowing = NewRdRMomentsFollowingClient(tx.config)
	tx.RdRUserFollowers = NewRdRUserFollowersClient(tx.config)
	tx.RdRUserFollowing = NewRdRUserFollowingClient(tx.config)
	tx.RdUser = NewRdUserClient(tx.config)
}
...
// applies a query, for example: RdMessage.QueryXXX(), the query will be executed
